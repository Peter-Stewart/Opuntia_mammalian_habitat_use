...
data{
  ...
  array[nsites] real grass; // Grass % cover (standardised)
  array[nsites] real forb; // Forb % cover (standardised)
  array[nsites] real shrub; // Shrub % cover (standardised)
  array[nsites] real succulent; // Succulent % cover (standardised)
...
model{
  ...
  beta_grass ~ normal(0,1);
  beta_forb ~ normal(0,1);
  beta_shrub ~ normal(0,1);
  beta_succulent ~ normal(0,1);
 ...
 // Gaussian process - non-centred
  ...
  for(isite in 1:nsites){
    ...
    grass[isite]*beta_grass[season[isite]] +
    forb[isite]*beta_forb[season[isite]] +
    shrub[isite]*beta_forb[season[isite]] +
    succulent[isite]*beta_succulent[season[isite]] +
    tree[isite]*beta_tree[season[isite]]);
